...
function draw() {
    ...
    gl.uniform2fv(shProgram.iTexTr, ppoint);

    lightModel.BufferData([0, 0, 0, 1, Math.sin(Date.now() * 0.001), 1])
    surface.Draw();
    ...
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false,
        m4.multiply(modelViewProjection,
            m4.translation(...getVertex(ppoint[0] * Math.PI * 2, ppoint[1] * Math.PI, 0.5, 1))));
    point.Draw();
}
...
function generatePointsNormal(u, v, a, b) {
    const steps = 30;
    const uStep = (1 / steps) * Math.PI;
    const vStep = (1 / steps) * Math.PI * 2;
    let v0 = getPoint(u, v, a, b);
    let v1 = getPoint(u + uStep, v, a, b);
    let v2 = getPoint(u, v + vStep, a, b);
    let v3 = getPoint(u - uStep, v + vStep, a, b);
    let v4 = getPoint(u - uStep, v, a, b);
    let v5 = getPoint(u - uStep, v - vStep, a, b);
    let v6 = getPoint(u, v - vStep, a, b);
...
function CreateSurfaceNormalData() {
    ...
    for (let i = 0; i <= steps; i++) {
        ...
        for (let j = 0; j <= steps; j++) {
            const v = (j / steps) * Math.PI * 2;
            const vStep = (1 / steps) * Math.PI * 2;
            generatePointsNormal(u, v, a, b);
            generatePointsNormal(u + uStep, v, a, b);
            generatePointsNormal(u, v + vStep, a, b);
            generatePointsNormal(u, v + vStep, a, b);
            generatePointsNormal(u + uStep, v, a, b);
            generatePointsNormal(u + uStep, v + vStep, a, b);
        }
    }
...
function CreateSurfaceTexCoordData() {
    ...
    for (let i = 0; i <= steps; i++) {
        ...
        for (let j = 0; j <= steps; j++) {
            const v = (j / steps) * Math.PI * 2;
            const vStep = (1 / steps) * Math.PI * 2;
            texCoordList.push(i / steps, j / steps)
            texCoordList.push((i + 1) / steps, j / steps)
            texCoordList.push(i / steps, (j + 1) / steps)
            texCoordList.push(i / steps, (j + 1) / steps)
            texCoordList.push((i + 1) / steps, j / steps)
            texCoordList.push((i + 1) / steps, (j + 1) / steps)
        }
    }
...
function initGL() {
    ...
    lightModel.BufferData([0, 0, 0, 1, 1, 1])
...
function LoadTexture() {
    ...
    image.src = "https://raw.githubusercontent.com/Maria-studyacc/VISUALIZATION-OF-GRAPHICAL-AND-GEOMETRIC-INFORMATION/PA2/tex.jpeg";
    image.onload = () => {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(
            gl.TEXTURE_2D,
...
window.onkeydown = (e) => {
    if (e.keyCode == 87) {
        ppoint[0] = Math.min(ppoint[0] + 0.02, 1);
    }
    else if (e.keyCode == 65) {
        ppoint[1] = Math.max(ppoint[1] - 0.02, 0);
    }
    else if (e.keyCode == 83) {
        ppoint[0] = Math.max(ppoint[0] - 0.02, 0);
    }
    else if (e.keyCode == 68) {
        ppoint[1] = Math.min(ppoint[1] + 0.02, 1);
    }
}